...
def ger_graph():
    edges=loadData1('D:\桌面\课程数据测试\师姐\data1000.csv') 
    B=nx.Graph()
    B=nx.Graph()
    nodes1=[]
    nodes2=[]
...
B.add_edges_from(edges)
print(B.size())
# adj=bipartite.biadjacency_matrix(B,nodes1,nodes2).toarray()
# print(adj.shape[0])
hidden_edges=random.sample(edges,711) #随机隐藏n%数量的边
# np.savetxt('D:\研究生资料\研一\课程数据测试\data_all\\隐藏边.txt',hidden_edges,fmt='%d')
# hidden_edges=loadData2('D:\研究生资料\研一\课程数据测试\data_all\隐藏边.txt')
# # #print(hidden_edges)
...
# print(pa)
key_list=ger_keylist(pa,82535,83148)#对键值进行节点切片
# print(len(key_list))
# print(len(key_list[0]))
pp1=ger_dic(key_list,pa)#对字典查找，建立新的切片
# print(pp1[0])
pre1=ger_pre_edge(1001,pp1,61)#课程数;新的切片字典;评分前n的键值
